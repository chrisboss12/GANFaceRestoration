...
def define_gan(g_model, dpatch_model, dglobal_model, in_shape=(256, 256, 3)):
    ...
    model.compile(loss=['binary_crossentropy', 'binary_crossentropy', 'mae'], optimizer=opt, loss_weights=[1, 1, 1000])
    model.summary()
    return model
...
def plot_mae_loss(glossmae, test=False):
    plt.plot(glossmae)
    plt.title('MAE Loss During Training')
    plt.xlabel('Epochs')
    plt.ylabel('MAE Loss')
    if test:
        plt.legend(['GAN test MAE loss'], loc='upper right')
        filename = '/mnt/c/Users/chris/2023-mcm-master/src/data/graphs_v2/GAN_test_mae_loss_graph_testing.png'
    else:
        plt.legend(['GAN train MAE loss'], loc='upper right')
...
def train(dpatch_model, dglobal_model, g_model, gan_model,
    ...
    loss_path = '/mnt/c/Users/chris/2023-mcm-master/src/data/logs/' + filename
    ...
    with open(loss_path, 'w') as fp:
        for i in range(start_epoch, n_epochs):
            ...
            glossmae_epoch.append(np.mean(glossmae))
            epochs.append(i)
            finish_time = time.time()
            print('d_real[%.5f] d_fake[%.5f] g_BCE[%.5f] d_real2[%.5f] d_fake2[%.5f] g_BCE2[%.5f] g_mae[%.5f] \n' %
                  (dlossreal_epoch[-1], dlossfake_epoch[-1], glossbce_epoch[-1], dlossreal2_epoch[-1], dlossfake2_epoch[-1],
                   glossbce2_epoch[-1], glossmae_epoch[-1]))
            fp.write('Epoch %d :d_real[%.5f] d_fake[%.5f] g_BCE[%.5f] d_real2[%.5f] d_fake2[%.5f] g_BCE2[%.5f] g_mae[%.5f] \n' %
            ...
            if (i + 1) % 5 == 0:
                ...
                for k in range(bat_per_epoch_test):
                    ...
                    # Update Generator weights
                    gloss_all_test, g_loss_BCE_test, g_loss_BCE2_test, g_loss_mae_test = gan_model.train_on_batch(damaged_images_test,
                                                                                              [y_real1, y_real2, X_real_test])
                    glossbce_test.append(g_loss_BCE_test)
                    glossbce2_test.append(g_loss_BCE2_test)
                    glossmae_test.append(g_loss_mae_test)
                ...
                glossmae_epoch_test.append(np.mean(glossmae_test))
                ...
                plot_mae_loss(glossmae_epoch_test, True)
    ...
    return dlossreal_epoch, dlossfake_epoch, glossbce_epoch, \
           dlossreal2_epoch, dlossfake2_epoch, glossbce2_epoch, glossmae_epoch, \
           dlossreal_epoch_test, dlossfake_epoch_test, glossbce_epoch_test, \
           dlossreal2_epoch_test, dlossfake2_epoch_test, glossbce2_epoch_test, glossmae_epoch_test
